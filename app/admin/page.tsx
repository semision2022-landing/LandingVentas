...
import type { Conversation, Agent } from '@/types/admin'
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      // Obtener agente actual y su rol
      ...
      if (isAgent) {
        // ── VISTA COMERCIAL: solo sus leads asignados ──────────────────
        ...
        const [
          ...
          supabase.from('conversations').select('*', { count: 'exact', head: true })
            .eq('assigned_to', agentId).gte('created_at', today),
          supabase.from('conversations').select('*', { count: 'exact', head: true })
            .eq('assigned_to', agentId).gte('created_at', today).not('visitor_email', 'is', null),
          supabase.from('conversations').select('*', { count: 'exact', head: true })
            .eq('assigned_to', agentId).gte('created_at', monthStart),
          supabase.from('conversations').select('*')
            .eq('assigned_to', agentId).order('created_at', { ascending: false }).limit(5),
        ])
        ...
        // Chart simplificado para agentes (solo sus conversaciones)
        ...
        const chartRows = await Promise.all(days.map(async (day) => {
          ...
          const { count: c } = await supabase.from('conversations')
    ...
    return (
      <div className="p-6">
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
          {Array.from({ length: isAgent ? 3 : 6 }).map((_, i) => (
  ...
  const metricCards = isAgent
    ? [
        { label: 'Leads asignados hoy', value: metrics.leadsToday, icon: <Users size={18} />, color: '#18224C', bgColor: '#EEF2FF' },
        { label: 'Chats asignados hoy', value: metrics.conversationsToday, icon: <MessageSquare size={18} />, color: '#5F4EDA', bgColor: '#F5F3FF' },
        { label: 'Leads este mes', value: metrics.conversationsMonth, icon: <TrendingUp size={18} />, color: '#579601', bgColor: '#F0FDF4' },
      ]
    : [
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      {/* Banner para comerciales */}
      {isAgent && (
        <div className="mb-6 px-5 py-4 rounded-2xl flex items-center gap-3"
          style={{ background: 'linear-gradient(135deg, #18224C 0%, #223870 100%)', color: 'white' }}>
          <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0"
            style={{ backgroundColor: '#00D0FF', color: '#18224C', fontWeight: 800, fontSize: 18 }}>
            {currentAgent?.name?.charAt(0) ?? '?'}
          </div>
          <div>
            <p className="font-bold text-sm">Bienvenido/a, {currentAgent?.name?.split(' ')[0]}</p>
            <p className="text-xs" style={{ color: 'rgba(255,255,255,0.6)' }}>
              Aquí ves solo tus leads asignados
            </p>
          </div>
        </div>
      )}
      ...
      {/* Metrics */}
      <div className={`grid gap-4 mb-6 ${isAgent ? 'grid-cols-1 sm:grid-cols-3' : 'grid-cols-2 lg:grid-cols-3'}`}>
      ...
      <div className="bg-white rounded-2xl" style={{ border: '1px solid #E2E8F0' }}>
        <div className="px-6 py-4 border-b flex items-center justify-between" style={{ borderColor: '#E2E8F0' }}>
          <h3 className="text-sm font-semibold" style={{ color: '#18224C' }}>
            {isAgent ? 'Mis últimas conversaciones' : 'Últimas conversaciones'}
          </h3>
        ...
        <div className="divide-y" style={{ borderColor: '#F1F5F9' }}>
          {conversations.length === 0 && (
            <p className="px-6 py-8 text-sm text-center" style={{ color: '#94A3B8' }}>
              {isAgent ? 'No tienes conversaciones asignadas aún' : 'No hay conversaciones aún'}
            </p>
          )}
