...
// Called by Vercel Cron every 30 min (and directly as GET for cron)
...
export async function GET() {
  ...
  if (cronSecret) {
    ...
    // For simplicity in GET cron calls we rely on the secret vercel injects
  }
  ...
  try {
    ...
    const campaignRows = campaigns.map((c) => {
      ...
      const costPerConv = ins?.cost_per_action
        ?.find((a) => ['purchase', 'lead', 'complete_registration'].includes(a.action_type))
      return {
